...
var $tabel_kelurahan='master_desa';
	...
	public function tampil_tahunmin(){
		$hsl=$this->db->query("SELECT MIN(periode) as periode from banyak_kendaraan WHERE status='0' ");
		return $hsl;
	}
	public function tampil_tahunmax(){
		$hsl=$this->db->query("SELECT MAX(periode) as periode from banyak_kendaraan WHERE status='0' ");
		return $hsl;
	}
